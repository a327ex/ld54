-- physics_tag can be any of the tags used with physics.set_collision_tags.
...
function collider:collider_draw(layer, color, line_width, z, fixed)
  if self.shape_type == 'rectangle' or self.shape_type == 'polygon' or self.shape_type == 'triangle' then
    layer:polygon({self.body:getWorldPoints(self.fixture:getShape():getPoints())}, color or colors.fg[0], line_width, z, fixed)
  elseif self.shape_type == 'edge' then
    layer:line(self.x + self.x1, self.y + self.y1, self.x + self.x2, self.y + self.y2, color or colors.fg[0], line_width, z, fixed)
  elseif self.shape_type == 'chain' then
    if self.loop then
      layer:polygon(self.vertices, color or colors.fg[0], line_width, z, fixed)
    else
      local points = {self.body:getWorldPoints(self.fixture:getShape():getPoints())}
      for i = 1, #points-2, 2 do
        layer:line(points[i], points[i+1], points[i+2], points[i+3], color or colors.fg[0], line_width, z, fixed)
        if self.loop and i == #points-2 then
          layer:line(points[i], points[i+1], points[1], points[2], color or colors.fg[0], line_width, z, fixed)
        end
      end
    end
  elseif self.shape_type == 'circle' then
    layer:circle(self.x, self.y, self.rs, color or colors.fg[0], line_width, z, fixed)
  end
end
...
-- Updates the .x, .y, .r attributes of this collider to match the physics body's. Useful to call before drawing something if you need its most recent physics position and angle
...
function collider:collider_set_position(x, y)
  ...
  return self:update_position_and_angle()
end
...
-- Call it after calling other movement functions.
...
-- Same as move_towards_point but does so only on the x axis
...
-- Same as move_towards_point but does so only on the y axis
...
-- Moves the object along an angle, most useful for simple projectiles that don't need any complex colliders
...
-- Same as rotate_towards_point except towards the mouse
...
-- Same as accelerate_towards_point except towards the mouse
...
-- Same as accelerate_towards_point except towards an object
